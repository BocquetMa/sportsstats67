<x-app-layout>
    <div class="min-h-screen bg-[#08090a] text-white p-6 pb-52"
         x-data="workoutManager({{ $workout->trainingSets->groupBy('exercise_id')->count() }}, '{{ csrf_token() }}')"
         x-init="init()">
                ...
                <div class="bg-[#111214] border border-slate-800 p-6 rounded-[2.5rem] mb-6">
                    <div class="flex items-center justify-between mb-4">
                        <div>
                            ...
                            <h3 class="text-2xl font-black" x-text="getCompletedCount() + ' / ' + getTotalSets()"></h3>
                        </div>
                ...
                @foreach($workout->trainingSets->groupBy('exercise_id') as $exerciseId => $sets)
                    <div @click="goToExercise({{ $loop->index }})"
                        ...
                        <div class="w-16 h-16 rounded-2xl overflow-hidden bg-black flex-shrink-0">
                            @if($sets->first()->exercise->image_url)
                                <img src="{{ $sets->first()->exercise->image_url }}"
                                     alt="{{ $sets->first()->exercise->name }}"
                                     class="w-full h-full object-cover"
                                     loading="lazy">
                            @else
                                <div class="w-full h-full flex items-center justify-center text-slate-700 text-xs">
                                    🏋️
                                </div>
                            @endif
                        </div>
        ...
        <div x-show="error"
             x-transition
             class="fixed top-24 left-1/2 transform -translate-x-1/2 z-50 bg-red-500 text-white px-6 py-3 rounded-2xl font-bold shadow-xl">
            <p x-text="error"></p>
        </div>

        <div x-show="isLoading"
             class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-40">
            <div class="bg-[#111214] border border-slate-800 p-8 rounded-3xl">
                <div class="animate-spin w-12 h-12 border-4 border-rose-500 border-t-transparent rounded-full mx-auto"></div>
                <p class="text-sm font-bold text-slate-400 mt-4">Sauvegarde...</p>
            </div>
        </div>

        <div class="fixed bottom-24 left-0 right-0 p-6 z-40">
            <form action="{{ route('workout.finish', $workout) }}" method="POST" onsubmit="return confirm('Terminer cette séance ? Les données seront sauvegardées.')">
                @csrf
                <button type="submit"
                        class="w-full bg-rose-500/10 border-2 border-rose-500/50 text-rose-500 py-4 rounded-3xl font-black uppercase italic text-sm tracking-widest transition-all hover:bg-rose-500 hover:text-white active:scale-95">
                    ✓ Terminer la Séance
                </button>
            </form>
        </div>
